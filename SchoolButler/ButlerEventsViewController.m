...
@synthesize eventIDArray, eventTitleArray, eventEndDateArray;
...
- (IBAction)handleAddButton:(id)sender
{
    ...
    NSString *message = [[NSString alloc] initWithFormat:@"Create a new Event?"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([eventIDArray count] > 0) {
        cell.textLabel.text = [NSString stringWithFormat:@"%@", [eventTitleArray objectAtIndex:indexPath.row]];
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%@", [eventEndDateArray objectAtIndex:indexPath.row]];
    } else {
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    {
        ButlerEventsDetailViewController *vc = [segue destinationViewController];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"kp_EventID" ascending:YES];
    ...
    if (objects == nil) {
        ...
        // compile local array
        eventIDArray = [[NSMutableArray alloc] initWithCapacity:1];
        eventTitleArray = [[NSMutableArray alloc] initWithCapacity:1];
        eventEndDateArray = [[NSMutableArray alloc] initWithCapacity:1];
        for (NSManagedObject *oneObject in objects) {
            [eventIDArray addObject:[oneObject valueForKey:@"kp_EventID"]];
            [eventTitleArray addObject:[oneObject valueForKey:@"eventName"]];
            
            NSDateFormatter *dateFormat = [[NSDateFormatter alloc] init];
            [dateFormat setDateFormat:@"MM/dd/yy"];
            NSString *dateString = [dateFormat stringFromDate:[oneObject valueForKey:@"endDate"]];
            [eventEndDateArray addObject:dateString];
        }
    }
